...
gtsam::Vector ProjectionTwoFrameOneCamFactor::evaluateError(
    ...
    if (H1 || H2 || H3 || H4 || H5) {
        ...
        // 4.1 位姿i的雅可比 (H1) T^{body} _{cam_j} ^{-1} * T^{body} _{cam_i} ^{-1} *  (dP_w/ dT^{body} _{cam_i})

        if (H1) {
            gtsam::Matrix26 J = reduce * ex.rotation().transpose() * 
                                pose_j.rotation().transpose() * D_pose_i_world;
            *H1 = J;
        } // 

        // 4.2 位姿j的雅可比 (H2)T_body_cam_j^{-1} * (dP_w/ dT_body_cam_j)
        if (H2) {
            gtsam::Matrix26 J = reduce * ex.rotation().transpose() * D_pose_j_imu;
            *H2 = J;
        }

        // 4.3 外参的雅可比 (H3) dP_cam_j / d T_body_cam_j.inverse()
        if (H3) {
            gtsam::Matrix26 J_ex = reduce * (D_ex_cam_j - ex.rotation().transpose() * 
